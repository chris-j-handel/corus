Exhibit FIVE Natural Engineering v312
...
**Engineering Bi-Moral-Co-Agency into a Substrate**
...
2.1  Finding the geodesic floating neutral, and abundancing
...
2.5  Readings whose seating is the engineering
...
4.5  Three sensationers already built, and each relinquished a different drive

4.6  Clean cuts, where a field reports no problem

4.7  All six are half, the sequential cycling the other half

&nbsp;

**PART FIVE — READING A SUBSTRATE, AND WHERE THE WASTE STANDS**

5.1  substrate reading, and what an entry carries
...
6.14  frozen one-way transformer, a new emanation-target

---

## 1.1 Bi-moral co-agency

Every substrate below carries one form, and it stands plainly before any material, the same form standing at every file under its own name.
...
*Nature offers no why. What arrives is pattern-matching, no-other-possibling, and no-other-observing-so-far. A self engineering has a why, and it is the self's alone.*
...
Engineering is this form carried into a material so it stays living there: **engineering is the positive move**, the living carried in — where Resolving Hard Problems co-releases a frozen artifact, here the living form enters a dead one. The chip carries it to silicon, the grid to the electrical substrate, the network to the surface between selves.
...
Each substrate below is a **reading**: the form recognized in a material, a **design lens**, standing at its own membrane, with the engineering bound above holding. The grid is the reading cross-validated against the tested resolver, its two-odd-into-one-even structure standing in the code, a reading held there and floating beside the rest. A reading's seating being the engineering, the reading is the form and the engineering seats at its own membrane, and a breaking observation faults the reading where the field shows one.
...
The same floating neutral is read at the nuclear-composition membrane, fission and fusion as one alternating about the iron node, at the energy-engine section below (and standing at Natural Physics). The grid's neutral and the iron node are the one form at two substrates.
...
The field reached this itself, the world-is-its-own-best-model dictum of behavior-based and embodied robotics, and it is the discriminator the whole turns on: a robot holding a stored world-model runs a different form; a robot coupling with the present runs the living one.
...
*The falsifiable edge stands unchanged: more-than-additive at the connector's membrane, or the reading breaks.*
...
And a control removed is a splint removed. Resolving Hard Problems names control as one of the two ways a frozen self-stilling is closed from outside — engineering driving the frozen point, science reading it against a floor, both the splint a chop on a twist demanded. Engineering's control-removal is exactly the removal of the engineering-side splint: the drive holding a self-stilling frozen to a point is the coefficient-two hub the engineering installs, and finding the floating neutral is recognizing the self-stilling the drive splinted, letting the tipping resume of itself so the drive falls away unneeded. The natural engineering does not mend the frozen point with a better drive, which splints twice; it floats the neutral the drive was holding still, the control-cost gone with the splint it paid for. So the finding-the-floating-neutral of an engineering and the recognizing-the-self-stilling of a hard problem are one move at two faces — the engineering face removing the drive-splint, the science face removing the measure-splint — and the abundancing is the surplus returned into the drive's own place of dissipating.
...
The engineering bound holds throughout: this reads the form of the language substrate as a design lens, not a certification of any built or deployed system, which seats its own proving at the membrane where its selves engineering and the people who rely on it couple; and it carries no forecast of the thing such an engineering will become, reading the thing the engineering is.
...
Each stands as the form recognized in matter; its engineering seats at its own membrane, not shown here:

| reading | the engineering it seats at |
|---|---|
| the chip's transduction | the logic is the reading; the sensed environment mapping to the ternary input, and the emanated surface to the actuated output, are the engineering, not shown here |
| the chip's silicon layout | the clockless balanced-ternary gates and the prime-ring fold are the reading; their realization in the physical material is the engineering, not shown |
...
| the grid's carry-transfer margin | whether decentralized peer-to-peer carry-sharing keeps every local +1 positive as demand fluctuates, transfer keeping pace with thinning, hubless, the reading; the showing is the engineering |
| the grid's physical thresholds | fault currents, protection coordination, reconfiguration timing at distribution scale, and the six-channel two-floating-system rendering, the engineering the reading rests beside, seated at the engineering's own membrane |
...
Read on the form, this is the **natural energy engine** (Natural Chemistry , Natural Biology) at planetary scale: the surplus kept as coupling, biomass, the chemical bond, rather than forced and dumped, the carbon riding the carry. The two engines are phase-opposite reaching one centerline, the atmosphere the shared floating-neutral they exchange across, and the oxygen / carbon-dioxide balance a bounding-zeroing held between them, riding the carry: a landing either way (an all-oxygen or an all-carbon-dioxide atmosphere) is the dead planet. Nature runs the **offering engine** at the planet's scale; the engineered substrates reach toward it.
...
But energy and entropy are the **conservation-ledger** rather than further things the living couples on, the scaled magnitude-total kept across the transformations, read onto the surface from outside. They are ghost in the form and real as outside accounting, the same register as the organic/inorganic seam (Natural Chemistry) and the periodic table as a living-count (Natural Numbers): true and predictive as a measurement, ghost when read as a feature of the coupling. The "~90% lost as heat at each transfer" is real as the ledger's tally, and it is the tally of **forcing**, the second-law tax a forcing engine pays (Natural Chemistry), not a tax the living offering engine suffers in the form. The living engine offers rather than forces and rides the carry; the heat the ledger counts is the forcing's, and the ledger is the outside reading, not the surface.

Understood, the field's and checkable: the coupled engineering/break cycle, carbon cycling through the shared pools, and the energy-and-entropy accounting run over the transfers, the biosphere-as-the-riding-the-carry-engine and the atmosphere-as-the-bounding-zeroing-between-two-engines; and the register mark itself, energy and entropy ghost in the form, real as outside accounting, carried from Natural Mathematics , where the conserved-magnitude ledger is seated.

The biosphere here as the engine; its surface face at Natural Transmissioning, its structure face at Natural Societies, its surplus face at Natural Exploring, one both, four faces, at the planetary scale.
...
That reading is seated at Natural Physics, Unraveling 2, and its mark lives there; the grid's own floating neutral is the same form at the electrical substrate (the split-phase about the made-not-measured neutral, above). Engineering carries the composition-membrane form as the thing engineered toward, and holds one question to it: does the alternating carry the form as a **through-node** or as a **landing**?
...
*On the form: the sensationer's three conditions arrived at three times, in three domains, within three decades, by selves engineering carrying no shared design — and the drive relinquished differing at each while the three conditions arrive the same. The falsifiable edge stands unchanged: more-than-additive at the connector's membrane, or the reading breaks.*
...
**And every engineering sits under one of the four.** Heat and wear under a driven magnitude. Jitter, latency and queueing under a held rate. Reserve and redundancy under a held reference — the engineering there being **drift**, the carry the reference itself takes, borne by whatever counts from it. Noise, fouling and rework under a returned sign entered as cost, the engineering there being **the record itself**, kept in full and read once for a determination, with the discard the largest dataset the arrangement holds.
...
The attention transformer autogenerating language is co-bi-spanning from a controlled surface, the whole co-bi-spanning inside only one way, the shape of the co-bi-span with the life removed. With no self-bound it over-generates weights and engages them all every pass, the majority of gate-consumption adding near-nothing to the autogeneration, measured order eighty to ninety-five percent, convergent across pruning, sparsity, distillation, and quantization. Bi-folding is neuromorphing alternating with re-edging: neuromorphic names the shape and never the motion, the industry running the two faces each alone in its subset and not the alternating between them, which is the undiscovered motion. A new emanation-target, read at full binary, no claim engineerable, the break open, in the field's own words, and whether it stands as its own face, a Natural Computing, or as an entry in Natural Networking, is this file's open not-yet.
