"""Assemble Exhibit THIRTY · Co-Chaining Logic Registry v371 from its parts."""
...
OUT = os.path.join(os.environ.get('CCL_WORK', os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'Exhibit_THIRTY_Co-Chaining_Logic_Registry_v371.md')
VERIFIER = 'Co_Chaining_Logic_Registry_verifier_v371.py'
...
def intro_fix(t):
    ...
    t = re.sub(r' \(resolver_v371\.py\)', '', t)
    t = t.replace('the resolver of resolver_v371.py', "Exhibit ONE v371's code")
    t = t.replace('against resolver_v371.py', "against Exhibit ONE v371's code")
    t = t.replace('the resolver file', "Exhibit ONE v371's code")
    ...
    assert 'ccl_' not in t and 'resolver_v371' not in t, [l for l in t.split('\n') if 'ccl_' in l or 'resolver_v371' in l][:3]
    return t
...
FIXES.append(("A link marked run was run against Exhibit ONE v371's code or by arithmetic, and the verifier holds each check.",
              "A link marked run was run against Exhibit ONE v371's code, by arithmetic or at the file's own text, and the verifier holds each check."))
...
doc = ['Exhibit THIRTY Co-Chaining Logic Registry v371', '', '&nbsp;', '', '# Co-Chaining Logic Registry', '',
