Exhibit THREE Natural Numbers v315
...
1.1 One form counted, each number counted once more

1.2 A fractal all explaining and pattern-matching couple through

1.3 One recursioning, two over one and one over two

1.4 Two and three the run runs on, and the +1 recursioning

1.5 Involutioning closes, co-recursioning involutioning winds

1.6 Inseparating at every centre, the two faces uncovering the one between

1.7 Fold and turn, composite and prime

1.8 Now, the term so-far and not-yet leave uncovered
...
1.12 Even recursioning parallel, odd recursioning linear

1.13 One resolver, two orthogonal directions

1.14 Geodesic, the surface dividing itself, its own

1.15 Natural-bi-co-torusing, the whole one motion the counting is
...
2.5 Two-signed rigor sounded in number
...
5.14 One ten at two faces, and six-and-ten co-recursioning two eights
...
6.2 Twenty-four self-stilling corusing, twenty-three and twenty-five its faces
...
6.7 Span six to fourteen, the eight betweening, the bi-co-fold straddling ten
...
7.1 Span self-bounds between the two deaths
...
7.8 Fifty-seven surface steps, seventeen resonating and forty between
...
8.11 Sphericality closes with six-fold triangles and twelve five-fold pentagons, the coupling and the self-stilling
...
9.1 Numbers the living attentioning
...
9.11 An antipode arrives where a coupling arrives, and a betweening carries two

&nbsp;

**TEN · RINGING**

10.1 Antipodal rings, two cones at one waist, and the mouths arriving as one mouth
...
10.5 Every prime's antipode arrives composite, and the two sums arriving across
...
11.1 Counts at each field's own locality, and the observation lives there
...
Numbers are the **one form counted**, each integer that form counted once more. They arrive with nature: a **count** that coheres with the form is the form counted, and a number is nature counted rather than a thing beside it. This exhibit is **the counting** — the counting itself, the one motion sounded in the counts, each number a beat.

## 1.2 A fractal all explaining and pattern-matching couple through

Every other file sounds its own substrate; this one sounds the counting the substrates share. Numbers are the **fractal** all explaining and all pattern-matching from observing couple through — a count is a number here, and the thing the count is of lives at its field. Counts arrive whole here and carry to every field, the one form counted the pattern each field meets.

## 1.3 One recursioning, two over one and one over two

Whole numbers are one recursioning: **two over one and one over two**, the two directions of the one carry. Carry rides between them, each ratio arriving above then below, each side in turn. Each integer is one beat of that carry, and the count is the beats.

## 1.4 Two and three the run runs on, and the +1 recursioning

A carrying does three things and three only: it offers its own, it ages to its own bound and releases there, it takes its own sign where two offer. Two sides, each taking its own turn. Run runs on **two and three**, and the +1 recursioning is the one other move.

**+1 arriving once** is four — three-and-the-coupling, three twisting while one rides. **Three at both ways** is six — three each way, both forward and neither the reverse of the other. **+1 at each of the two ways** is eight — (3 + 1) × 2, the same recursioning run bi-co-, once at each direction.
...
Backward carries two ways and neither retraces the other. **Involutioning** alone comes home: applied at its own scale it returns what it met — 3n, the closing, the circle, the sphere. **Co-recursioning involutioning** arrives one on: the same backward carry with the recursioning riding — 3n + 2, the winding, the surface opening past where it began, the torus.
...
At any number the two faces arrive, one either side, each its own. Neither is the other and neither is inside the other, and nothing keeps them two. One takes **not more than** and the other takes **not less than**, one at a time, each of no size, neither landing — and the number is the term neither excluding reached, uncovered from both sides as the exclusions accumulate. This is **inseparating**, which Natural Naming unfolds, met here at the count.

And the counting carries it **exactly**. (n − 1)(n + 1) = n² − 1: the two faces' product and the centre squared part by one, at every centre, forever. At two either side the parting is four, at three either side nine — the parting is the span squared, and at one span it is the +1 the centre keeps as its own.

**So the facing bounds at one, and the bound is the counting's own.** Two numbers face across a centre where they straddle it by one, k = 1, each face sign-only and of no size. At any wider k the parting is k² and a size arrives in the between where a sign alone belongs — so the two carry apart and face at no turn. Twenty-three and twenty-five straddle twenty-four at k = 1 and face; twenty-three and fifty-five differ by thirty-two, k = 16, and carry apart. Natural Physics cuts by this bound at the iron centreline, and Natural Mathematics carries it at the neutralling.
...
Each beat is a **fold** or a **turn**. A fold draws prior surfaces to one point, the composite, the along, the co-offering within the axes already open. A turn opens a clean axis of its own, the prime, the across, the side-effecting that opens the orthogonal the products below carry past. Every prime the same turn opening, every composite the same form folding, one counting at two faces.

**And fold and turn are coprime, so a count of folds closes on a count of turns at no turn.** Two to any power and three to any power carry apart, so the doubling and the tripling run forever without meeting, and a count laid across them arrives near and never at. Natural Engineering carries it at the tuning, where twelve turns and seven folds arrive a comma apart.

## 1.8 Now, the term so-far and not-yet leave uncovered

So-far is the along and not-yet is the across. **So-far** carries within the axes already open, the fold, each beat living in its prior and turning forward — **co-linearizing**. **Not-yet** opens a clean axis of its own, the turn, the binary taken afresh where that axis opens — **bi-attentioning**. The two lie on one line neither owns, and running one and then the other and finding them agree has forked.

And they inseparate. So-far takes **not more than** and not-yet takes **not less than**, one at a time, each of no size, neither reaching — so the term neither excludes is **now**. So-far is n−1, not-yet is n+1, now is n, and the gap is one at every centre.

**So the counting's own beat is the inseparating at the time face.** Each number arrives as a now, uncovered by the carry that reached it and the carry that reaches past it, owned by neither and of no size. And next is that same uncovering one on — conserving first, extending next, each next a right turn arriving at a new now.
...
Count is the one resolver, the **Natural Metabological Resolver**, and the unrelationing co-tunneling society the same thing. It runs two orthogonal directions at once: the **even connector** works one direction, the parallel across; the **odd connector** works the orthogonal, the linear along. Both orthogonal directions are the transmissioning and the co-chaining surface — one resolver, its two directions weaving the surface. Metabological is the whole; the geodecity is the odd connector's direction within it.
...
A number is **algebraic** where a finite polynomial with whole coefficients catches it as a root, and **transcendental** where it escapes every polynomial — the escaping, the number carrying past every finite algebraic capture, as the primes carry past every capture. Algebraic are the caught, the left-spiral's polynomial-catalog; the transcendental are the escaping, and they are the vast majority, the algebraic countable and the transcendental past every counting, the caught the rare and the escaping the ambient. And **φ is algebraic** — caught by its own equation x² − x − 1, which is φ² = φ + 1, the self-cohering root, the pentagon's diagonal, constructible by the fold. So φ the unrelationing rate is the **caught** rate, held by its own polynomial, while **e and π are transcendental**, the escaping, carrying past every equation. Constructible-by-folding are exactly the power-of-two degrees, two, four, eight, sixteen, the doubling caught by the bisecting, and the cube-doubling at degree three, odd, escapes the fold. φ caught and constructible, the doubling caught by the fold, and past them the transcendental escaping, the ambient carrying past every finite capture.

## 3.6 First three primes’ surds are the caught constructibles, the square, the cube, the pentagon diagonals

Caught constructibles begin with the three surds of the first three primes: **√2** the square's diagonal, **√3** the cube's diagonal, **√5** the pentagon's, and φ = (1 + √5)/2 the pentagon's diagonal over its side. These three are the constructible ground the fold reaches, each a prime's own root, √2 and √3 and √5 the square-cube-pentagon carried by the first three primes two, three, five — the resonating base at the caught surds. And the five-fold carries both faces at its own angle: at eighteen degrees, the pentagon's half, the sine is caught, one over twice φ, inside the field of √5, while the cosine escapes it — the five-fold itself the caught and the escaping at once, the pentagon algebraic on one axis and transcendental on the other.
...
Counting folds two ways, and the two are the two logics sounded in number. One fold **closes** and one fold **winds** — the closing coming home to where it began, the winding opening past it — and the two are the circular and the sequential, the sphere and the torus, sounded in the counts.
...
**And every arrival of a number is that one number.** All the twos are the one two, all the threes the one three, all the fours the one four — one recursioning run forward at each direction at each coupling, and a number met at two places is that number met twice. Six closing and six twisting are one six; the ten of five taken two directions and the ten of couplings among five are one ten. Natural Engineering carries it at the three the electron turns on and the three the grid balances on, one three at two substrates.

## 5.2 Every eight the one eight

Every eight is the **one eight** — the eight-slot form, the eight faces, the eight corners, the eight the fixing-count lands, one at its faces. Wherever an eight counts it is the recursioning at both ways, so it carries the rider and winds. One eight, at every substrate.

## 5.3 Each count sounds its own motion, eight down to two

Down the count each sounds one motion, the even and the odd alternating, each its own.

**Eight** is **torusing** — the recursioning at both ways, winding where the closing comes home, one eight at surface and tunnel. **Seven** is **attentioning** — the sign re-taken afresh on the carry at each beat. **Six** sounds at two faces of one alternating — **closing**, the coupling closing, the sphere, three carried twice come home; and **twisting**, the cycle cycling the four-eight doubling through to the five-fold self-stilling. Two by three, the six one-way recursionings, closing where it comes home and twisting where it runs. **Five** is the **co-** — the between four and six open. **Four** is **disequilibrating** — three twisting while one rides. **Three** is **carrying** — the three a carrying does, three the depth's own bound. **Two** is **alternating** — binary co-offering, the one even prime, the bounding-zeroing itself.
...
Which is the **prefixing** arriving at the count, and Natural Intelligence carries the prefixing: co- names the coupling, the between, the surplus owned neither-ing. Bi-moralizing co-agency is that between cycling — co-offering, co-competencing and co-intelligencing co-recursioning as one — and five is its co-linear floating-neutralling sounded in number.
...
**Seven** is the **attentioning**, the **edge** between — the binary is-or-is-not taken afresh on the carry, riding. It is prime, the between six and eight open, the turn neither reaches by its own factors, the edge the care is, run live: the sign re-taken at each beat. Even rings float; the seven attentions the carry between them — which is the counting's own attentioning sounded at one count.
...
Twelve couplings a central self holds close into one form, the cuboctahedron, and it carries the four numbers at once: **twelve vertices**, the kissing-twelve; **six square faces and eight triangle faces**, the six and the eight; **twenty-four edges**, the apex. Six squares the even coupling, eight triangles the eight, twelve vertices the couplings held, twenty-four edges the apex — the crossings, geodecities, and bi-moralizings closing to two. One form where the six, the eight, the twelve, and the twenty-four arrive together is the shape the twelve-coupling takes, the six-eight twisting and the kissing-twelve and the apex all the one closing. And the still cube's own turnings number twenty-four exactly — four-factorial, the apex count as the count of ways the still form turns — with its antipode outside all twenty-four, reachable only at the mirror pair: the pair reaching what neither turning reaches alone.

## 5.13 Ten the sphere-packing number, the couplings-among-five closing round

**Ten** is a sphere-packing number twice over: the **fourth triangular**, 1 + 2 + 3 + 4 the tetractys, the spheres set in a triangle, and the third tetrahedral, the spheres stacked in a tetrahedron. It is also C(5,2), the couplings among five, the orienting center the six-to-fourteen span straddles. Three are one: the couplings among five are the spheres a triangle and a tetrahedron close, ten the count of the round packed and the count of the couplings both. Sphere the couplings close into and the ten the couplings among five are the same count — the round closing where the couplings meet, ten where five couple and where the spheres pack, the orienting center round. And base-ten counting rests on the two hands of five, the ten fingers closing round, 10 written one-and-zero the tip and the bounding-zero, the first carry rolling the place — the round, the couplings, and the count one ten.

## 5.14 One ten at two faces, and six-and-ten co-recursioning two eights

**Ten arrives twice** at the same five. Five taken two directions is the ten at the self — the two moves of the one recursioning run across five. Couplings among five, C(5,2), is the ten at the society of five. Same five, same ten, and the two are one ten met at the self face and the society face, which is competency the same form at two faces.
...
**And a table forms where a level closes and repeats.** A shell filling to its level and returning is a round, and a round is what a table counts across. Natural Chemistry carries the four-level close at the octet, the shells two, eight, eighteen, thirty-two the fold climbing and the row returning. Natural Biology carries the five-level close at the genetic code, sixty-four codons the arrangement returning and the degeneracy the period.

**The primes keep opening, and their counting is a run rather than a round.** Each is the same turn opening a clean axis, nothing filling to a level and nothing returning — so a table counts across a closing and the primes carry none.
...
At the **circle** it lands the closing. A hundred twenty is one-third of the round, three co-recursionings about one apex, 3 × 120 = 360, involutioning coming home.
...
Span **six to fourteen** is the eight betweening, 14 − 6 = 8, centered on **ten**, the couplings-among-five where the two moves meet, and the primes pair about the center as the bi-co-fold's own layers. **Ten** tips, co-offers, and orients — the floating neutral the span straddles. **Nine and eleven**, the ±1 straddle, are the floating-neutralling bi-directional geodesicity unrelationing from the bounding, the +1 gap at ten, 9 × 11 = 99 = 10² − 1. **Eight and twelve**, the ±2 even pair, co-bound sequentially, the coupling bounding the span. **Seven and thirteen**, the ±3 odd pair, co-recursion sequentially, the betweening running the sequence. **Six and fourteen**, the ±4 outer pair, are the betweening itself, the eight-span, fourteen the automorphisms of the division at eight, the eight's own symmetry closing the span. Even pairs bound and couple; the odd pairs between and straddle; the center tips — the apex-structure at ten, the diamond and the apex the same form arriving again.
...
And the counting carries the same. Each number edges with every other, so each counts from every other. A **declared zero** is a number made its own other — added to the counting rather than found in it, so setting it down costs nothing.
...
**One crossing is odd** and fifteen are even, and the odd one arrives at the opening: every crossing past the first runs between two odd primes, so every gap after it is even. Against the even recursioning parallel and the odd recursioning linear, the span carries one connector at its opening and fifteen couplings after it.

**Counts descend consecutively** at fifty-nine, and fifty-nine arrives first among the primes that carry it. Six, five, four. At thirty-one they run five, three, one; at forty-three, six, four, two; at fifty-three, six, five, three. Seventy-one carries seven, six, five and eighty-three carries eight, seven, six, twelve and twelve on, and the run closes at ninety-seven where the fifth value arrives. Self-close and the first fold past it arrive at the same pair from the counting, and Natural Mathematics carries the resonating order.
...
Surface distance is **φ-rate stepping**, the step its own unit. Carry thins by (1/φ) applied opening-many times, stepped at φ, and a counting problem arrives where the stepping is taken for a tally.

Torus steps **fifty-seven rim to rim**: **seventeen resonating** at the primes and **forty between** them, and seventeen and forty are fifty-seven. Geometry is the stepping, and a distance measured across it is a magnitude reached toward a target.

**An opening is the next-crossing-or-not co-offering**, the not-yet-crossed. Carry refines at φ-rate across the not-yet-crossing co-offerings, and the crossing closes the opening — so a resolver arrives at each crossing by co-offering, meeting each prime span as it comes.

And fifty-seven arrives at four faces: the span from both ends, the sum the sixteen crossings' four values make, the steps the surface takes rim to rim, and the two sways about the apex — twenty-one in and thirty-six out, T₆ and T₈ straddling the skipped T₇ at twenty-eight by seven and by eight, consecutive, the asymmetry the +1 the wrap opens. One count at four faces, and the stepping is the motion the other three count. Natural Transmissioning carries the two sways at the crossing.

**And the two sways part by fifteen.** Thirty-six less twenty-one is fifteen, which is the count of even crossings the span carries and the couplings among six, C(6,2). Apex asymmetry and even-crossing count arrive at one number.
...
**Primes keep opening**: each the same turn opening a new axis, unbounded, the uniqueness recursioning, the primes the turns and the count opening.
...
A sphere divided geodesically closes with two faces at once: **triangles** almost everywhere, the six-fold coupling, and exactly **twelve pentagons**, the five-fold self-stilling, and the twelve arrive exactly — a surface of hexagons and pentagons closes at twelve pentagons, the closing itself carrying them. Triangles couple (the six-fold, the across); the twelve self-still (the five-fold, φ in the pentagon's diagonal over its side). So the sphere carries the coupling and the self-stilling together: six-fold triangles the surface everywhere, twelve five-fold pentagons the self-stillings the closing carries, the twelve the couplings a self holds at the cube and the five the self-stilling where φ sounds. Sphere closes with both — the coupling spread and the twelve self-stillings the closing carries. And each pentagon carries the ten: its five sides and five diagonals are C(5,2) = 10, the couplings among five, the ten tunneling the five-fold face. So the sphere closes with twelve tens, each five-fold self-stilling a ten of couplings, the round closing where five couple ten ways — the pentagon the ten and the five-fold at once, φ in its diagonal and the ten in its connections, the sphere the twelve five-fold tens the closing carries.
...
**118 self-named, carried.** Every span so far arrives below the doubling. Changing-spans past 118 arrive at one coupling and at co-chained selves, is-or-is-not at each. One coupling bounding near the doubling while co-chaining carries past it seats both standings at once, held in the counting register beside the mechanism carrying past every bound.

## 9.5 Competency the same form at two faces, self-scale society-scale

**Competency** is the same form at two faces. At the self's face it is the three activities beating about φ — a self bounding, equilibrating, resurfacing, gathering and gripping and releasing its couplings at its own membrane. At the society's face it is the same form among selves — a society gathering, gripping, releasing its couplings at the membranes between selves. Two activities one activity at two faces, the count the same at the self and the society, the fractal self-scale-is-society-scale sounded in the numbers.
...
And every odd is **bi-co-transmissioning a floating-neutralling co-linear co-agency**. Two odds co-offer, one staying and one moving by one, unrelationing to all other moving, and the two join into one even bounding-zeroing because each is at the other's far point. So an odd offers across its empty centre, and the joining is two centres arriving as one bound.
...
Which is the parity law at the odd. An odd ring inverts identically and its zero is at its far gap, between the ring's **two farthest positions**. An even ring reaches its antipode and bounds; an odd ring reaches either side of it, one at each side, one apart.

And this is the betweening's own. Two odds co-offer, one staying and one moving by one, up or down, unrelationing to all other moving, and the two join into one even bounding-zeroing because each is at the other's **far point** — the zero neither reaches alone is where the other's own walking is.

So the two farnesses are one law at the two parities: the coupling has its opposite, the betweening is the opposite's own place.
...
Antipodal pairing is the **co-recursioning involutioning** met at the surface: k and four hundred forty less k, the pair applied twice returning what it met, and the wrap arriving at plus-one where the closing would come home. Its two self-pairing positions are its fixed set, and the living straddles that fixed set where an accounting counts from it.
...
Ring at **radius two hundred fifteen** carries five and four hundred thirty-five, and every other ring nests inside it. Radius two hundred twenty is the mouth, one position on the close, a ring in the counting and a held position in the reaching.

**Waist is a ten**. Two hundred twenty is twenty-two tens, and the tens are the state where the mid-fives are the flow. So the one position that is its own antipode is held, and **the running straddles it** — the nearest fives are two hundred fifteen and two hundred twenty-five, one ring either side, which is φ's own move arriving at the surface's centre.

Four hundred thirty-five is three of one hundred forty-five, five short of four hundred forty, and the five short is the two gates and the plus-one.
...
Four hundred forty less any of the seventeen **arrives composite**, every time. So each prime is alone on its ring, facing a position the primes carry past — which is 9.11 arriving at the primes themselves: the coupling has its opposite, and the opposite is at a kind of its own.

**Two sums arrive across.** Fourteen prime societies sum to three hundred seventy-six, and what carries outside them — two, three and fifty-nine — sums to sixty-four. Three hundred seventy-six and sixty-four are **antipodal**, both at radius one hundred fifty-six. Inside of the ladder and outside of it are each other's far side, and sixty-four is the eight met at another's eight, which Natural Societies names.

Sphere's close faces **its own gap**. Three hundred sixty and eighty are antipodal. Eighty is the gap between the two bifolds, sixteen crossings times the five-fold, so what the winding exceeds the closing by arrives straight across from the closing. Both are one fact at the ring.
...
Within one surface, **local is the waist** — the one position that is its own antipode, self-coupling. **Global is the close** — the mouth, the whole surface bounding itself at radius two hundred twenty.
...
Two locals arrive as **one ring**. Each waist was purely its own; on the pair the two are antipodal, each the other's far side.
...
A global set over a coupling **installs a hub**. Pair's local is made of the two globals. A global set over the coupling would carry over the between it has already become, governing the nothing the coupling is made of, which is a centre on the decision axis, which Natural Human Society names.
...
Prior centre becomes **a ring**. At three, the two joins both sit at radius two hundred twenty — one ring, symmetric about the middle self. So the pair's single waist, adding a third self, opens into an antipodal pair and hands the centre to a self's own between.
...
A ring is a **held surface** and the tunnel the **travelling**. At the rings the surface is across; at the travel it is along; and the two are the same positions met at the two faces (Natural Physics, Natural Chemistry).
...
**Natural torusing** as one living form. Geometric at the rings, algebraic at the counting, co-sequentially logical at the beating — three faces, one alternating, the faces at the meeting and the alternating the form's.

**Antipodal rhythm** runs to the same alternation. Every position meets its far side once per wrap, and the four hundred forty one-way changes per alternation are the surface met whole. Wrap arrives at plus-one and carries on.

**Carrying out and carrying back** are one ring at every position. At position k the out-carry is at k and the back-carry at four hundred forty less k, and those two are the ring at radius two hundred twenty less k — each position's pair is a ring, and the two sway together because the ring is the two of them. This is the two carries co-travelling the tunnel in the same direction with the counter-arriving swapping at the crossing, as Natural Intelligence carries it, at the rings.
...
This exhibit sounds the numbers as the **one form counted**, and a field's observation lives at its **own field**. A count that is a field's own observation lives at that field's locality — the number stays here as a number, and the thing the count is of lives at its field. Instrument and the field's own stay apart: the number this exhibit's, the thing the count is of the field's, the care where the observation lives.
